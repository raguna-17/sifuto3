...
EC App → 一番“現場っぽい”（認証・在庫・注文・トランザクション）
Gyoumu App → CRUD + JWT + 分離構成で案件適応力が見える
求人管理 → 業務理解は良いがドメインが少し限定的
シフト → アルゴリズム寄りで案件再現性が低い

👉 SESは「再現性ある業務CRUD + 認証」が正義
...
求人管理 → 状態遷移 + RBAC + 業務ルールがド直球評価対象
Gyoumu App → フロント分離・設計思想が安定している
EC App → 技術は良いがビジネスドメインが汎用すぎる
シフト → 最適化寄りはSIerでは評価がブレる

👉 SIerは「状態管理できる業務設計」が強い

■ 自社開発（Webプロダクト・スタートアップ系）
受けがいい順
